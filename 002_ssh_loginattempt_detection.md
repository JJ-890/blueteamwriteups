...
> **Project Status:** Phase 1 — Infrastructure & Initial Detection **COMPLETE**
> Phase 2 — Alerting & Severity **IN PROGRESS**
> Phase 3 — Automated Response **PLANNED**
> Phase 4 — Dashboard & Investigation **PLANNED**
> Phase 5 — Documentation & Detection Testing **PLANNED**

## TL;DR

I built a Python script (`ssh_monitor.py`) to automatically detect failed SSH login attempts on a Linux server I set up in Oracle Cloud.

The script ran successfully but produced no alerts. Instead of immediately assuming the detection logic was broken, I built a debug version of the script to trace the problem step by step. This revealed that the issue wasn't a detection-logic bug at all — it was a **Linux file-permissions issue** preventing the script from reading the authentication log.

After correcting the log access, the detection started working as expected. The script successfully detected both my own controlled SSH login tests and **real unsolicited SSH traffic** hitting the internet-facing server.

Detected events were then stored as structured JSON alerts in `alerts.json`, providing a foundation for future alerting, severity classification, automated response, and investigation capabilities.

### Biggest Takeaway

> **A detection rule can be logically correct and still fail if it can't reach its telemetry. Always troubleshoot the entire detection pipeline — not just the detection logic.**

This phase established the foundation for the rest of the **Blue Team Automation & Detection Lab**. The project will continue beyond SSH detection with additional alerting, response automation, investigation, dashboarding, and structured detection testing.
...
<img src="screenshots/realorginalssh.png" alt="Initial ssh screen" width="900">
